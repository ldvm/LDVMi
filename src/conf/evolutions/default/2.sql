...
create table "VISUALIZERS_COMPATIBILITY" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"VISUALIZATION_ID" BIGINT,"VISUALIZER_ID" BIGINT,"DATASOURCE_ID" BIGINT,"CREATED" TIMESTAMP,"MODIFIED" TIMESTAMP);
...
alter table "VISUALIZERS_COMPATIBILITY" add constraint "VC_VISUALIZATION_FK" foreign key("VISUALIZATION_ID") references "VISUALIZATIONS"("ID") on update NO ACTION on delete NO ACTION;
alter table "VISUALIZERS_COMPATIBILITY" add constraint "VC_VISUALIZER_FK" foreign key("VISUALIZER_ID") references "VISUALIZERS"("ID") on update NO ACTION on delete NO ACTION;
