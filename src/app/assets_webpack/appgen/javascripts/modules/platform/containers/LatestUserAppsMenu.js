...
class LatestUserApps extends Component {
  ...
  render() {
    ...
    return  (
      <IconMenu {...buttonPositioning} iconButtonElement={<Button label="My applications" inverted />}>
        <Link to={createAppRoutes.createAppUrl()} style={noDecorationStyle}>
          <MenuItem primaryText="Create new application" />
        </Link>
        <Divider />
        ...
        <Link to={dashboardRoutes.dashboardUrl()} style={noDecorationStyle}>
          <MenuItem primaryText="All applications" />
        </Link>
      </IconMenu> 
    );
  }
