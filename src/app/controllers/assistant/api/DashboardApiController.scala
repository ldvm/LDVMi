...
import model.assistant.entity.Visualizer.Visualizer
import model.assistant.entity.{Discovery, UserDataSource, UserDataSourceId, UserPipelineDiscoveryId}
import model.assistant.repository.UserDataSourcesRepository
import model.assistant.rest.AddVisualizerRequest._
import model.assistant.rest.UpdateVisualizerRequest._
...
import model.assistant.rest.UpdateDataSourceRequest.UpdateDataSourceRequest
import model.assistant.service.{ApplicationsService, DiscoveriesService, VisualizerService}
...
class DashboardApiController(implicit inj: Injector) extends SecuredRestController {
  ...
  def deleteDataSource(id: Long) = RestAction[EmptyRequest] { implicit request => json =>
    withUserDataSource(UserDataSourceId(id)) { userDataSource =>
      // Here we're also deleting just the user meta data which is part of assistant. The actual
