...
trait ComponentTemplateService extends CrudService[ComponentTemplateId, ComponentTemplate, ComponentTemplateTable, ComponentTemplateRepository] {
  ...
  def getAllForDiscovery(maybeDs: Option[(String, Seq[String])] = None, combine: Boolean, name: Option[String] = None)(implicit session: Session): (Map[ComponentType, Seq[SpecificComponentTemplate]], Option[DataSourceTemplate])
