...
import model.assistant.entity.{InstallResult, UserId}
...
import model.assistant.InstallBundle
import model.assistant.rest.UpdateVisualizerRequest.UpdateVisualizerRequest
...
class InstallService(implicit inj: Injector) extends Injectable {
  ...
  private def installDataSource(dataSource: (String, String))(implicit session: Session) = {
    ...
    // visualization assistant (we use the admin user with id 1 as the data source owner).
  ...
  private def installVisualizer(visualizer: (String, String, String, String))(implicit session: Session) = {
    // Install an assistant Visualizer which means that we take an appropriate LDVM
