...
class GeoServiceImpl(implicit val inj: Injector) extends GeoService with SessionScoped with Injectable {
  ...
  def quantifiedThings(evaluation: PipelineEvaluation, thingsUrls: Seq[String], valueConnections: Seq[String], placeConnections: Seq[String], limit: Int): Option[Seq[QuantifiedThing]] = {
    ...
    sparqlEndpointService.getResult(evaluationToSparqlEndpoint(evaluation), new QuantifiedThingQuery(maybeThingsUrls, maybeValueConnections, maybePlaceConnections, maybeLimit), new QuantifiedThingExtractor)
  }
  ...
  def quantifiedPlaces(evaluation: PipelineEvaluation, placeUrls: Seq[String], placeTypes: Seq[String], valueConnections: Seq[String], limit: Int): Option[Seq[QuantifiedPlace]] = {
    ...
    sparqlEndpointService.getResult(evaluationToSparqlEndpoint(evaluation), new QuantifiedPlaceQuery(maybePlaceUrls, maybePlaceTypes, maybeValueConnections, maybeLimit), new QuantifiedPlaceExtractor)
  }
}
