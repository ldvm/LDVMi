        ...
        ['$scope', 'DataCubeService', '$q', '$location', '$routeParams',
            function ($scope, DataCubeService, $q, $location, $routeParams) {
                ...
                /*                var URI_rdfType = "http://www.w3.org/1999/02/22-rdf-syntax-ns#type";
                 ...
                 var URI_dimension = "http://purl.org/linked-data/cube#dimension";
                 var URI_measure = "http://purl.org/linked-data/cube#measure";
                 ...
                 ng.forEach($scope.dataStructures[$scope.selectedDataStructure].dimensions, function (dim, dimUri) {
                 var negative = dim.order == 1;
                 if (map[dim.uri]) {
                 ng.forEach(dim.values, function (val) {
                 ...
                 ng.forEach(dimension.values, function (value) {
                 ...
                 /* DataCubeService.get({queryName: "list-cubes", visualizationId: $routeParams.id},
                 function (data) {
                 ...
                 ng.forEach(container, function (def, uri) {
                 ...
                 var res = {labels: []};
                 ng.forEach(value[URI_binding], function (b) {
                 var solution = data[b.value];
                 var o;

                 if (solution[URI_variable][0].value == "o") {
                 o = solution[URI_value][0];
                 if (o.type == 'uri') {
                 res.uri = o.value;
                 ...
                 ng.forEach($scope.dataStructures, function (dsd, i) {
                 if (i != $index) {
                 dsd.active = false;
                 }
                 });
                 ...
                 if (val[URI_binding]) {
                 var res = {};
                 ng.forEach(val[URI_binding], function (b) {
                 var solution = data[b.value];
                 ...
                 ng.forEach($scope.highcharts.series, function (s) {
                 ...
                 ng.forEach($scope.ticks, function (tArray) {
                 ...
                 ng.forEach(valuesCopy, function (v) {
                 ...
                 ng.forEach(dataQueue, function (x) {
                 ...
                 ng.forEach($scope.dataStructures, function (val, key) {
                 ...
                 ng.forEach(component.values, function (componentVal) {
                 ...
                 ng.forEach(node[URI_component], function (component) {
                 if (component.type == "bnode" || component.type == "uri") {
                 var componentValue = data[component.value];

                 ng.forEach(queue, function (queueItem) {
